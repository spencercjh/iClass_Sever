...
/**
 * Servlet implementation class CountOneStudentAllCheck_AllTypes
 */
@WebServlet("/CountOneStudentAllCheck_AllTypes")
public class CountOneStudentAllCheck_AllTypes extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CountOneStudentAllCheck_AllTypes() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int num_all = 0;
			if (resultset.next()) {
				num_all = resultset.getInt("ALLCHECK");
			}
			// 输出结果
			System.out.println(student_id + " all checkin in the " + subject_id + " is:" + num_all);
			out.println(URLEncoder.encode(String.valueOf(num_all), "UTF-8"));
			// 关闭连接
			resultset.close();
			conn.close();
			statement.close();
